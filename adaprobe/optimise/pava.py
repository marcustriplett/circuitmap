...
def simultaneous_isotonic_regression(X, Ys, y_min=0, y_max=1):
    ...
    Y_preds = vmap(_isotonic_regression, in_axes=(
        0, None))(Y_s, jnp.ones_like(Y_s[0, :]))
...
def _isotonic_regression(y, weight):
    ...
    def true_fun(args):
        ...
        with loops.Scope() as s:
            ...
            s.j = s.i
            for _ in s.while_range(lambda: s.j < s.k + 1):
                s.solution = index_update(
                    s.solution, s.j, s.numerator / s.denominator)
                s.j += 1
            s.pooled = 1
            ...
            for _ in s.while_range(lambda: s.i < s.n):
                ...
                for _ in s.while_range(
                    lambda: jnp.logical_and(
                        s.k < s.n, s.solution[s.k] >= s.solution[s.k + 1])
                ):
                    s.k += 1
                args = (s.i, s.k, s.solution, s.numerator,
                        s.denominator, s.pooled)
