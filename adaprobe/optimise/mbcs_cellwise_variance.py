...
def mbcs_cellwise_variance(obs, I, mu_prior, beta_prior, sigma_prior, phi_prior, phi_cov_prior, iters=50, 
	...
	hist_arrs = [mu_hist, beta_hist, lam_hist, sigma_hist, phi_hist, phi_cov_hist, z_hist]
...
def update_lam(y, I, mu, beta, lam, N, phi, phi_cov, lam_mask, key, num_mc_samples, N):
...
def update_sigma(y, mu, lam, z):
	...
	for n in range(N):
		...
		constr += np.sum(lam[n, spk_locs]) * sigma[n]**2
	return sigma, constr
