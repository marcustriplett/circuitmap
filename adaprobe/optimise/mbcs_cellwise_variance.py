...
def mbcs_cellwise_variance(obs, I, mu_prior, beta_prior, sigma_prior, phi_prior, phi_cov_prior, iters=50,
    ...
    sigma_hist = jnp.zeros((iters, N))
    phi_hist = jnp.zeros((iters, N, 2))
    phi_cov_hist = jnp.zeros((iters, N, 2, 2))
    z_hist = jnp.zeros((iters, K))

    hist_arrs = [mu_hist, beta_hist, lam_hist,
                 sigma_hist, phi_hist, phi_cov_hist, z_hist]
    ...
    for it in tqdm(range(iters), desc='CAVI', leave=False):
        ...
        lam, key = update_lam(y - z, I, mu, beta, sigma, lam,
        ...
        sigma, constr = update_sigma_proportion_weight(
            mu, lam, scale=sigma_scale)
...
def _adaptive_excitability_threshold(y, mu, lam, phi, shape, rate, lam_mask, max_iters=20, init_thresh=0.2, scale_factor=0.95, min_thresh=0.09,
    ...
    for it in range(max_iters):
        ...
        no_presynaptic_events = np.all(
            _lam[np.where(_mu != 0)[0]] < 0.5, axis=0)
...
@jit
def update_beta(lam, beta_prior):
    return 1/jnp.sqrt(jnp.sum(lam, 1) + 1/(beta_prior**2))
...
def update_mu_constr_l1(y, mu, Lam, constr, penalty=1, scale_factor=0.5, max_penalty_iters=10, max_lasso_iters=100,
...
def update_lam(y, I, mu, beta, sigma, lam, phi, phi_cov, lam_mask, key, num_mc_samples, N):
    ...
    with loops.Scope() as scope:
        ...
        for n in scope.range(N):
            ...
            scope.lam = index_update(scope.lam, n, lam_mask * (I[n] > 0) * sigmoid(scope.mcE - 1/(
                2 * sigma[n]**2 + 1e-5) * scope.arg))  # require spiking cells to be targeted
...
def update_sigma_residual_psc_variance(y, _mu, _lam, z):
    ...
    for n in connected_cells:
        ...
        mu_excl = mu[excl]
        sigma[n] = np.std(y_spk - lam_excl.T @ mu_excl -
                          z[spk_locs]) if spk_locs.shape[0] > 0 else 0.
...
@jit
def update_sigma_gamma_normal(y, mu, beta, lam, shape_prior, rate_prior):
    K = y.shape[0]
