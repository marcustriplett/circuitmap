...
@njit
def cavi_online_spike_and_slab_no_ots(yk, stim, mu_prev, beta_prev, alpha_prev, shape_prev, rate_prev, phi_map_prev, phi_cov_prev, omega, C,
	...
	"""
	tark, Ik = stim # target neuron and power
	if seed is not None:
		np.random.seed(seed)
	...
	for it in range(iters):
		...
		lamk = update_lamk_monte_carlo(yk, mu, beta, alpha, lamk, shape, rate, phi_map, phi_cov, mask, 
			omega, tark, Ik, C, num_mc_samples=num_mc_samples)
		shape, rate = update_sigma(yk, mu, beta, alpha, lamk, shape_prev, rate_prev)
		phi_map, phi_cov = update_phi(lamk, phi_map_prev, phi_cov_prev, omega, Ik, C, 
...
@njit
def update_phi(lamk, phi_prior, phi_cov_prior, omega, Ik, C, newton_steps=10, tol=1e-8, init_t=1e2, t_mult=1e1, t_loops=10, verbose=False):
	...
	for n in range(N):
		...
		for j in range(t_loops):
			for st in range(newton_steps):
				...
				phi_new, phi_cov_new = _backtracking_newton_step_with_barrier(phi[n], lamk[n], t, phi_prior[n], phi_cov_prior[n], omega[n], Ik, C[n], verbose=verbose)
...
@njit
def _backtracking_newton_step_with_barrier(phi, lamk, t, phi_prior, phi_cov_prior, omega, Ik, C, backtrack_alpha=0.25, 
	backtrack_beta=0.5, max_backtrack_iters=15, verbose=False):
	"""Newton's method with backtracking line search. For fixed neuron n.
	"""
	H_inv = np.zeros((2, 2))
	fk = sigmoid(phi[0] * Ik - phi[1])
	...
	for it in range(max_backtrack_iters):
		lhs = _negloglik_with_barrier(phi + step * v, t, lamk, Ik, phi_prior, phi_cov_prior_inv, phi_cov_prior_inv_det)
		rhs = _negloglik_with_barrier(phi, t, lamk, Ik, phi_prior, phi_cov_prior_inv, phi_cov_prior_inv_det) \
...
@njit
def _negloglik_with_barrier(phi, t, lamk, Ik, phi_prior, phi_cov_prior_inv, phi_cov_prior_inv_det):
