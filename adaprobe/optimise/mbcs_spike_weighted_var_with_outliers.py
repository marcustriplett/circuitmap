...
def mbcs_spike_weighted_var_with_outliers(y_psc, I, mu_prior, beta_prior, shape_prior, rate_prior, iters=50,
    ...
    # Setup lam mask
    lam_mask = (np.array([np.correlate(y_psc[k], y_psc[k])
                for k in range(K)]).squeeze() > y_xcorr_thresh)
    lam_mask[np.max(y_psc, axis=1) < lam_mask_fraction *
             np.max(y_psc)] = 0  # mask events that are too small
    ...
    for it in tqdm(range(iters), desc='CAVI', leave=True):
        ...
        for _ in range(lam_iters):
            lam = update_lam_with_isotonic_receptive_field(
                y, I, mu, beta, lam, shape, rate, lam_mask, update_order, spike_prior, num_mc_samples, N)

        receptive_field, spike_prior = update_isotonic_receptive_field(lam, I)
        mu, lam = isotonic_filtering(mu, lam, I, receptive_field, minimum_spike_count=minimum_spike_count,
                                     minimum_maximal_spike_prob=minimum_maximal_spike_prob + spont_rate)
        shape, rate = update_noise(
            y, mu, beta, lam, noise_scale=noise_scale, num_mc_samples=num_mc_samples_noise_model)
...
def inner_newton(y, spks, mask, weights, lam, t, max_backtrack_iters, backtrack_alpha, backtrack_beta, eps=1e-5):
    ...
    for bit in range(max_backtrack_iters):
        lhs = objective_with_barrier(
            y, weights, spks + step * search_dir, lam, t, mask)
        rhs = objective_with_barrier(
            y, weights, spks, lam, t, mask) + backtrack_alpha * step * J @ search_dir
        # go condition
        cond = jnp.min(jnp.array([(lhs > rhs) + jnp.isnan(lhs), 1]))
...
inner_newton_vmap = vmap(inner_newton, in_axes=(
...
def backtracking_newton_with_vmap(y, spks, tar_matrix, weights, lam_mask, newton_penalty=1e2, iters=20, barrier_iters=5, t=1e0, barrier_multiplier=1e1,
    ...
    for barrier_it in range(barrier_iters):
        for it in range(iters):
            spks = inner_newton_vmap(y, spks, tar_matrix, weights, newton_penalty,
                                     t, max_backtrack_iters, backtrack_alpha, backtrack_beta).T
...
def update_lam_backtracking_newton(y, lam, tar_matrix, mu, lam_mask, shape, rate, penalty=1, scale_factor=0.5, max_penalty_iters=10,
    ...
    for it in range(max_penalty_iters):
        ...
        if err <= constr:
            ...
            if verbose:
                print(
                    ' !!! converged without meeting constraint on iteration: %i !!!' % it)
            break
        else:
...
def update_noise(y, mu, beta, lam, noise_scale=0.5, num_mc_samples=10, eps=1e-4):
    ...
    s_samps = (np.random.rand(num_mc_samples, N, K)
               <= lam[None, :, :]).astype(float)
    mc_ws_sq = np.mean([(w_samps[i] @ s_samps[i]) **
                       2 for i in range(num_mc_samples)], axis=0)
    mc_recon_err = np.mean(
        [(y - w_samps[i] @ s_samps[i])**2 for i in range(num_mc_samples)], axis=0)
...
def update_mu_constr_l1(y, mu, lam, shape, rate, penalty=1, scale_factor=0.5, max_penalty_iters=10, max_lasso_iters=100,
    ...
    lasso = Lasso(alpha=penalty, fit_intercept=False, max_iter=max_lasso_iters,
                  warm_start=warm_start_lasso, positive=positive)
    ...
    for m in range(N):
        ...
        if mu[n] != 0:
            ...
            arg = -2 * y * mu[n] + 2 * mu[n] * jnp.sum(jnp.expand_dims(mu[mask], 1) * lam[mask], 0) \
                + (mu[n]**2 + beta[n]**2)
            lam = index_update(lam, n, lam_mask * (I[n] > 0) * (
                mu[n] != 0) * sigmoid(spike_prior[n] - shape/(2 * rate) * arg))
...
def update_lam(y, I, mu, beta, lam, shape, rate, phi, phi_cov, lam_mask, update_order, key, num_mc_samples, N):
    ...
    with loops.Scope() as scope:
        ...
        # scope.key_next needs to be initiated outside of loop
        ...
        for m in scope.range(N):
            ...
            scope.lam = index_update(
                scope.lam, n, lam_mask * (I[n] > 0) * sigmoid(scope.mcE - shape/(2 * rate) * scope.arg))
