...
@njit
def cavi_online_spike_and_slab_3d_omega(yk, stim, mu_prev, beta_prev, alpha_prev, shape_prev, rate_prev, phi_map_prev, phi_cov_prev, Omega, C,
	interp=0.5, iters=15, verbose=True, newton_steps=15, seed=None, lam_update='monte-carlo', num_mc_samples=10, init_t=1e1, t_mult=15, 
	t_loops=3, focus_radius=30):
...
def update_lamk_monte_carlo(yk, mu, beta, alpha, lamk, shape, rate, phi_map, phi_cov, mask, Omega, Lk, Ik, C, focus, num_mc_samples=5):
	...
	for n in focus:
		...
		for indx in range(num_mc_samples):
			mk = Ik * np.exp(-(Lk - C[n]) @ Omega[n] @ (Lk - C[n]).T)
...
def _backtracking_newton_step_with_barrier(phi, lamk, t, phi_prior, phi_cov_prior, Omega, Ik, Lk, C, backtrack_alpha=0.25, 
	...
	"""
	mk = Ik * np.exp(-(C - Lk) @ Omega @ (C - Lk).T)
	H_inv = np.zeros((2, 2))
