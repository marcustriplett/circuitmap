...
# mu, beta, alpha, lamk, zetak, etak, phi_map, phi_cov = optimise.cavi_online_spike_and_slab_no_ots(
# 			obs, stimuli, self.state['mu'], self.state['beta'], self.state['alpha'], self.state['phi_map'], 
# 			self.state['phi_cov'], self.state['omega'], self.state['sigma'], self.cell_locs, **fit_options

def cavi_online_spike_and_slab_spike_noise(yk, stim, mu_prev, beta_prev, alpha_prev, phi_map_prev, phi_cov_prev, omega, sigma, rho, C, 
	...
	for it in range(iters):
		beta[focus] = update_beta(alpha, lamk, zetak, etak, sigma, beta_prev, focus)
		mu = update_mu(yk, mu, beta, alpha, lamk, zetak, sigma, mu_prev, beta_prev, mask, focus)
		alpha = update_alpha(yk, mu, beta, alpha, lamk, zetak, etak, sigma, alpha_prev, mask, focus)
		lamk = update_lamk_monte_carlo(yk, mu, beta, alpha, lamk, zetak, etak, sigma, phi_map, phi_cov, mask, 
		...
		zetak = update_zetak(yk, mu, alpha, lamk, zetak, etak, rho, sigma, mask, focus)
		etak[focus] = update_etak(mu, beta, alpha, lamk, rho, sigma, focus)
...
@njit
def update_zetak(yk, mu, alpha, lamk, zetak, etak, rho, sigma, mask, focus):
	for n in focus:
		zetak[n] = (etak[n]**2) * (1/(sigma**2) * yk * mu[n] * alpha[n] * lamk[n] \
			- 1/(sigma**2) * mu[n] * alpha[n] * lamk[n] * np.sum(mu[mask[n]] * alpha[mask[n]] * lamk[mask[n]] * zetak[mask[n]])
			+ 1/(rho**2))
	return zetak

@njit
def update_etak(mu, beta, alpha, lamk, rho, sigma, focus):
	return 1/np.sqrt(1/(sigma**2) * (mu[focus]**2 * beta[focus]**2) * alpha[focus] * lamk[focus] + 1/(rho**2))

@njit
def update_beta(alpha, lamk, zetak, etak, sigma, beta_prev, focus):
	return 1/np.sqrt(1/(sigma**2) * alpha[focus] * (zetak[focus]**2 + etak[focus]**2) * lamk[focus] + 1/(beta_prev[focus]**2))

@njit
def update_mu(yk, mu, beta, alpha, lamk, zetak, sigma, mu_prev, beta_prev, mask, focus):
	for n in focus:
		mu[n] = (beta[n]**2) * (1/(sigma**2) * yk * alpha[n] * lamk[n] * zetak[n] \
			- 1/(sigma**2) * alpha[n] * lamk[n] * zetak[n] * np.sum(mu[mask[n]] * alpha[mask[n]] * lamk[mask[n]] * zetak[mask[n]]) \
			+ mu_prev[n]/(beta_prev[n]**2))
	return mu

@njit
def update_alpha(yk, mu, beta, alpha, lamk, zetak, etak, sigma, alpha_prev, mask, focus):
	for n in focus:
		arg = -2 * yk * mu[n] * lamk[n] * zetak[n] + 2 * mu[n] * lamk[n] * zetak[n] * np.sum(mu[mask[n]] * alpha[mask[n]] * lamk[mask[n]] * zetak[mask[n]]) \
		+ (mu[n]**2 + beta[n]**2) * lamk[n] * (zetak[n]**2 + etak[n]**2)
		alpha[n] = sigmoid(np.log((alpha_prev[n] + EPS)/(1 - alpha_prev[n] + EPS)) - 1/(2 * sigma**2) * arg)
	return soften(alpha)

@njit
def update_lamk(yk, mu, beta, alpha, lamk, zetak, etak, sigma, fk, mask, focus):
	for n in range(focus):
		arg = -2 * yk * mu[n] * alpha[n] * zetak[n] + 2 * mu[n] * alpha[n] * zetak[n] * np.sum(mu[mask[n]] * alpha[mask[n]] * lamk[mask[n]] * zetak[mask[n]]) \
		+ (mu[n]**2 + beta[n]**2) * alpha[n] * (zetak[n]**2 + etak[n]**2)
		lamk[n] = sigmoid(np.log((fk[n] + EPS)/(1 - fk[n] + EPS)) - 1/(2 * sigma**2) * arg)
	return lamk
...
@njit
def update_lamk_monte_carlo(yk, mu, beta, alpha, lamk, zetak, etak, sigma, phi_map, phi_cov, mask, omega, Lk, Ik, C, focus, num_mc_samples=5):
