...
@njit
def cavi_offline_spike_and_slab(y, stim, mu_prior, beta_prior, alpha_prior, shape_prior, rate_prior, phi_map_prior, phi_cov_prior, omega, C, 
	init_t=1e4, t_mult=1e1, t_loops=10, iters=10, verbose=False, newton_steps=10, seed=None, lam_update='monte-carlo', num_mc_samples=5, return_parameter_history=False):
	...
	mask = get_mask(N)
	lam = update_lam_monte_carlo(y, mu, beta, alpha, lam, shape, rate, phi_map, phi_cov, mask, omega, L, I, C, num_mc_samples=num_mc_samples)
	...
	for it in range(iters):
		...
		if lam_update == 'MAP':
			lam = update_lam_MAP(y, mu, beta, alpha, lam, shape, rate, phi_map, omega, I, L, C, mask)
		elif lam_update == 'monte-carlo':
			lam = update_lam_monte_carlo(y, mu, beta, alpha, lam, shape, rate, phi_map, phi_cov, mask, omega, L, I, C, num_mc_samples=num_mc_samples)
		shape, rate = update_sigma(y, mu, beta, alpha, lam, shape_prior, rate_prior)
		phi_map, phi_cov = update_phi(lam, phi_map_prior, phi_cov_prior, omega, I, L, C, init_t=init_t, t_mult=t_mult, t_loops=t_loops, 
...
@njit
def update_lam_monte_carlo(y, mu, beta, alpha, lam, shape, rate, phi_map, phi_cov, mask, omega, L, I, C, num_mc_samples=5):
...
@njit
def update_phi(lam, phi_prior, phi_cov_prior, omega, I, L, C, newton_steps=10, tol=1e-8, init_t=1e2, t_mult=1e1, t_loops=10, verbose=False):
	...
	for n in range(N):
		...
		for j in range(t_loops):
			for st in range(newton_steps):
				...
				phi_new, phi_cov_new = _backtracking_newton_step_with_barrier(phi[n], lam[n], t, phi_prior[n], phi_cov_prior[n], omega[n], I, L, C[n], verbose=verbose)
...
@njit
def _backtracking_newton_step_with_barrier(phi, lam, t, phi_prior, phi_cov_prior, omega, I, L, C, backtrack_alpha=0.25, 
	...
	for it in range(max_backtrack_iters):
		lhs = _negloglik_with_barrier(phi + step * v, t, lam, m, phi_prior, phi_cov_prior_inv, phi_cov_prior_inv_det)
		rhs = _negloglik_with_barrier(phi, t, lam, m, phi_prior, phi_cov_prior_inv, phi_cov_prior_inv_det) \
...
@njit
def _negloglik_with_barrier(phi, t, lam, m, phi_prior, phi_cov_prior_inv, phi_cov_prior_inv_det):
