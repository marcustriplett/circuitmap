...
def _cavi_sns(y, I, mu_prior, beta_prior, alpha_prior, lam, shape_prior, rate_prior, phi_prior, phi_cov_prior,
    ...
    for it in trange(iters):
        ...
        mu, key = update_mu(y, mu, beta, alpha, lam, shape,
                            rate, mu_prior, beta_prior, N, key)
        alpha, key = update_alpha(
            y, mu, beta, alpha, lam, shape, rate, alpha_prior, N, key)
        ...
        if noise_update == 'iid':
            shape, rate = update_sigma(
                y, mu, beta, alpha, lam, shape_prior, rate_prior)
        elif noise_update == 'trial-wise':
            shape, rate, key = update_noise(
                y, mu, beta, alpha, lam, key, noise_scale=noise_scale, num_mc_samples=num_mc_samples)
        else:
            raise Exception
        ...
        if it > phi_thresh_delay:
            ...
            z = update_z_l1_with_residual_tolerance(
                y, alpha, mu, lam, lam_mask, scale_factor=scale_factor, penalty=penalty)
    ...
    mu, beta, alpha, lam, z = reconnect_spont_cells(
        y, I, lam, mu, alpha, beta, z, minimax_spk_prob=minimax_spk_prob, minimum_spike_count=minimum_spike_count)
...
def reconnect_spont_cells(y, stim_matrix, lam, mu, alpha, beta, z, minimax_spk_prob=0.3, minimum_spike_count=3):
    ...
    while len(disc_cells) > 0:
        ...
        if pava >= minimax_spk_prob and spike_count >= minimum_spike_count:
            ...
            print('Reconnecting cell %i with maximal pava spike rate %.2f' %
                  (focus, pava))
            z_locs = np.intersect1d(
                np.where(stim_matrix[focus])[0], np.where(z)[0])
...
def update_noise(y, mu, beta, alpha, lam, key, noise_scale=0.5, num_mc_samples=10):
    ...
    w_samps = (mu + std * jax.random.normal(key,
               [num_mc_samples, N])) * alpha_samps
    key, _ = jax.random.split(key)
    ...
    mc_ws_sq = jnp.mean(
        jnp.sum((w_samps[..., jnp.newaxis] * s_samps)**2, axis=1), axis=0)
    mc_recon_err = jnp.mean(
        (y - jnp.sum(w_samps[..., jnp.newaxis] * s_samps, axis=1))**2, axis=0)
...
def _eval_spike_rates(stimv, lamv, powers):
    ...
    with loops.Scope() as scope:
        ...
        for p in scope.range(npowers):
            ...
            mask = (locs >= 0)
            sr = jnp.sum(lamv[locs] * mask) / \
                (jnp.sum(mask) + 1e-4 * (jnp.sum(mask) == 0.))
...
def update_isotonic_receptive_field(lam, stim_matrix, powers, mu, alpha, minimax_spk_prob=0.3, minimum_spike_count=3, disc_strength=0.):
    ...
    receptive_fields = simultaneous_isotonic_regression(
        powers, inf_spike_rates)
    disc_cells = jnp.logical_or(
        receptive_fields[:, -1] < minimax_spk_prob, jnp.sum(lam, axis=1) < minimum_spike_count)

    alpha = alpha * (1. - disc_cells) + disc_strength * disc_cells
    mu = mu * (1. - disc_cells) + disc_strength * disc_cells
    lam = lam * ((1. - disc_cells)[:, jnp.newaxis]) + \
        (disc_strength * disc_cells)[:, jnp.newaxis]
...
def update_z_l1_with_residual_tolerance(y, _alpha, _mu, _lam, lam_mask, penalty=2e1, scale_factor=0.5, max_penalty_iters=50, verbose=False,
    ...
    if verbose:
        print(
            ' ==== Updating z via soft thresholding with iterative penalty shrinking ==== ')
    ...
    with loops.Scope() as scope:
        ...
        for m in scope.range(N):
            ...
            scope.mask = jnp.unique(
                jnp.where(scope.all_ids != n, scope.all_ids, jnp.mod(n - 1, N)), size=N-1)
...
def update_alpha(y, mu, beta, alpha, lam, shape, rate, alpha_prior, N, key):
    ...
    with loops.Scope() as scope:
        ...
        for m in scope.range(N):
            ...
            scope.alpha = index_update(scope.alpha, n, sigmoid(
                jnp.log((alpha_prior[n] + EPS)/(1 - alpha_prior[n] + EPS)) - 1/2 * scope.arg))
    key, _ = jax.random.split(key)
...
def update_lam(y, I, mu, beta, alpha, lam, shape, rate, phi, phi_cov, lam_mask, key, num_mc_samples, N, powers, minimum_spike_count, minimax_spk_prob, it, phi_thresh_delay):
    ...
    with loops.Scope() as scope:
        ...
        for m in scope.range(N):
            ...
            # require spiking cells to be targeted
            est_lam = lam_mask * (I[n] > 0) * \
                sigmoid(scope.mcE - 1/2 * scope.arg)

            # check pava condition
            srates = _eval_spike_rates(I[n], est_lam, powers)
            pava = (_isotonic_regression(srates, jnp.ones_like(
                srates))[-1] >= minimax_spk_prob) * (jnp.sum(est_lam) >= minimum_spike_count)
            pava = pava * (it > phi_thresh_delay) + \
                1. * (it <= phi_thresh_delay)
