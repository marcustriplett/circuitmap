...
@njit
def cavi_online_spike_and_slab_multistim(yk, stim, mu_prev, beta_prev, alpha_prev, shape_prev, rate_prev, phi_map_prev, phi_cov_prev, omega, C,
	...
	for j in range(Lk.shape[0]):
		focus[np.where(np.sqrt(np.sum(np.square(C - Lk[j]), 1)) <= focus_radius)[0]] = 1
	focus = np.where(focus > 0)[0]
...
def update_lamk_monte_carlo(yk, mu, beta, alpha, lamk, shape, rate, phi_map, phi_cov, mask, omega, Lk, Ik, C, focus, num_mc_samples=5):
	...
	for n in focus:
		...
		for indx in range(num_mc_samples):
			mk = np.sum(Ik * np.exp(-omega[n] * np.sum(np.square(Lk - C[n]), 1)))
...
def _backtracking_newton_step_with_barrier(phi, lamk, t, phi_prior, phi_cov_prior, omega, Ik, Lk, C, backtrack_alpha=0.25, 
	...
	"""
	mk = np.sum(Ik * np.exp(-omega * np.sum(np.square(C - Lk), 1)))
	H_inv = np.zeros((2, 2))
