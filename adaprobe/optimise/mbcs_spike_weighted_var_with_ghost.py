...
def mbcs_spike_weighted_var_with_ghost(obs, I, mu_prior, beta_prior, shape_prior, rate_prior, phi_prior, phi_cov_prior, iters=50,
    ...
    for it in tqdm(range(iters), desc='CAVI', leave=False):
        ...
        lam = update_lam_with_isotonic_receptive_field(
            y, I, mu, beta, lam, shape, rate, lam_mask, update_order, spike_prior, num_mc_samples, N, spont_rate)
...
def isotonic_filtering(mu, lam, I, isotonic_receptive_field, minimum_spike_count=1, minimum_maximal_spike_prob=0.2):
    ...
    disc_locs = np.where(
        isotonic_receptive_field[:-1, -1] < minimum_maximal_spike_prob)[0]
...
def update_isotonic_receptive_field(lam, I):
    ...
    # final row associated with ghost cell, leave zero
...
def update_lam_with_isotonic_receptive_field(y, I, mu, beta, lam, shape, rate, lam_mask, update_order, spike_prior, num_mc_samples, N, spont_rate):
    ...
    for m in range(N):  # N does not include ghost
        ...
        lam = index_update(
            lam, n, lam_mask * (I[n] > 0) * sigmoid(spike_prior[n] - shape/(2 * rate) * arg))

    # Update ghost cell
    mask = jnp.array(np.arange(N)).squeeze()
    arg = -2 * y * mu[N] + 2 * mu[N] * jnp.sum(jnp.expand_dims(mu[mask], 1) * lam[mask], 0) \
        + (mu[N]**2 + beta[N]**2)
    # ghost can spike on any trial
    lam = index_update(lam, N, lam_mask *
                       sigmoid(spont_rate - shape/(2 * rate) * arg))
