...
def simulate(N=300, T=900, H=10, trials=1000, nreps=1, connection_prob=0.05, powers=[45, 55, 65], min_latency=160, gamma_beta=1.5e1, sigma=6e-4,
			frac_strongly_connected=0.2, strong_weight_lower=20, strong_weight_upper=40, weak_exp_mean=4, min_weight=9, phi_0_lower=0.2, phi_0_upper=0.25,
	...
	if kernel is None:
		...
		psc_kernels = get_psc_kernel(tau_r, tau_d, T)
	...
	# population_pscs = kernel_conv_trialwise(psc_kernels, spk_times.astype(int), spks, mult_noise, weights)
	...
	psc = psc + spont_pscs + gp_noise + noise
...
def simulate_continuous_experiment(N=100, expt_len=int(2e4), gamma_beta=1.5e1, min_latency=60, powers=[45, 55, 65],
	...
	strong_weight_upper=40, weak_exp_mean=4, min_weight=7, phi_0_lower=0.2, phi_0_upper=0.25, phi_1_lower=10, phi_1_upper=15, kernel=None, phi_0=None, phi_1=None,
	H=10, nreps=1, connection_prob=0.1, spont_rate=3, kernel_window=3000, prior_context=100, ground_truth_eval_batch_size=1000, max_power_min_spike_rate=0.3):
	'''Simulate continuous mapping experiment (to be sliced and reshaped post-hoc).
	'''
	...
	# pad spikes to ensure min spike rate at maximal power is met, provided sufficiently many no-spike trials exist
	max_power = np.max(powers)
	for n in range(N):
		locs = np.where(stim_matrix[n] == max_power)[0]
		fr = np.mean(spks[n, locs])
