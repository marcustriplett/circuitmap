...
from holoprobe.optimise.utils import get_filt_grid_around_loc_multi, get_psf_func, get_gauss_func, nrelu
...
class Simulation3d:
	def __init__(self, dimx=125, dimy=125, dimz=100, spacing=10, grid_density=5, N=16, a=0.5, sigma=3, Omega=None, phi_0=None,
		phi_1=None, min_w=3, max_w=20, mode='online', num_filter_pts_per_dim=4, filter_dim_len=20):
		""" Initialise a 3d adaprobe simulation object.
		"""
		self.cell_locs = _generate_3d_locs(dimx, dimy, dimz, N, spacing)
		self.cell_grids = get_filt_grid_around_loc_multi(self.cell_locs, num_points=num_filter_pts_per_dim, dim=filter_dim_len)
		self.cell_psfs = [get_psf_func(self.cell_grids[n]) for n in range(N)]

		# gaussian filter evaluated on each cell's grid
		gauss_eval = [get_gauss_func(self.cell_grids[n])(self.cell_locs[n], 1e-2*np.eye(3)) for n in range(N)]
		self.cell_filters = np.c_[np.array([nrelu(gauss_eval[n] + np.random.normal(0, 0.1, num_filter_pts_per_dim**3)) for n in range(N)]), 4 + 3*np.random.rand(N)]

		dropout_prob = 0.5
		mask = 1.0 - (np.random.rand(self.cell_filters.shape[0], self.cell_filters.shape[1]) <= dropout_prob)
		mask[:, -1] = 1.0
		self.cell_filters *= mask
	...
	def next_trial(self, L, I):
		...
		"""
		stim = np.array([np.c_[self.cell_psfs[n](np.array([I]), np.array([L])), -np.ones(1)] for n in range(self.N)]).squeeze() # squeeze is hacky here, but sufficient for now
		fr = _sigmoid(np.array([stim[n] @ self.cell_filters[n] for n in range(self.N)])).squeeze() 
