...
@njit
def cavi_online_spike_and_slab(yk, stim, mu_prev, beta_prev, alpha_prev, shape_prev, rate_prev, phi_map_prev, phi_cov_prev, omega, C,
	interp = 0.5, iters=10, verbose=True, newton_steps=5, seed=None, lam_update='monte-carlo', num_mc_samples=5, init_t=1e2, t_mult=1e1, t_loops=10):
	...
	for it in range(iters):
		beta = update_beta(alpha, lamk, shape, rate, beta_prev)
		mu = update_mu(yk, mu, beta, alpha, lamk, shape, rate, mu_prev, beta_prev, mask)
		alpha = update_alpha(yk, mu, beta, alpha, lamk, shape, rate, alpha_prev, mask)
		if lam_update == 'MAP':
			fk = sigmoid(phi_map[:, 0] * Ik * np.exp(-omega * np.sum(np.square(Lk - C), 1)) - phi_map[:, 1])
			lamk = update_lamk(yk, mu, beta, alpha, lamk, shape, rate, fk, mask)
		elif lam_update == 'monte-carlo':
			lamk = update_lamk_monte_carlo(yk, mu, beta, alpha, lamk, shape, rate, phi_map, phi_cov, mask, omega, Lk, Ik, C, num_mc_samples=num_mc_samples)
		shape, rate = update_sigma(yk, mu, beta, alpha, lamk, shape_prev, rate_prev)
		phi_map, phi_cov = update_phi(lamk, phi_map_prev, phi_cov_prev, omega, Ik, Lk, C, newton_steps=newton_steps, init_t=init_t, t_mult=t_mult, t_loops=t_loops)
...
def update_beta(alpha, lamk, shape, rate, beta_prev):
	return 1/np.sqrt(shape/rate * alpha * lamk + 1/(beta_prev**2))
...
@njit
def update_lamk(yk, mu, beta, alpha, lamk, shape, rate, fk, mask):
	N = mu.shape[0]
	for n in range(N):
...
@njit
def update_lamk_monte_carlo(yk, mu, beta, alpha, lamk, shape, rate, phi_map, phi_cov, mask, omega, Lk, Ik, C, num_mc_samples=5):
	"""Infer latent spike rates using Monte Carlo samples of the sigmoid coefficients.
	"""
	N = lamk.shape[0]
	for n in range(N):
		arg = -2 * yk * mu[n] + 2 * mu[n] * np.sum(mu[mask[n]] * alpha[mask[n]] * lamk[mask[n]]) + (mu[n]**2 + beta[n]**2)
...
@njit
def update_phi(lamk, phi_prior, phi_cov_prior, omega, Ik, Lk, C, newton_steps=10, tol=1e-8, init_t=1e2, t_mult=1e1, t_loops=10, verbose=False):
